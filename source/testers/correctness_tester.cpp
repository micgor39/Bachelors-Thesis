...
std::pair<int, int> correctness_tester::generate_random_operations(
    solution *model, solution *tested, int number_of_operations) {
    ...
    for (int i = 0; i < number_of_operations; i++) {
        if (query_type == 1) {
            ...
            tested_labels.push_back(
                tested->concat(tested_labels[index1], tested_labels[index2]));
            ...
            bool tested_output =
                tested->equals(tested_labels[index1], tested_labels[index2]);
            ...
            bool tested_output =
                tested->smaller(tested_labels[index1], tested_labels[index2]);
            if (model_output == tested_output) {
                passed_tests++;
            } else if (debug) {
                std::cerr << model_output << " " << tested_output << "\n";
            ...
            int tested_output = tested->longest_common_prefix(
                tested_labels[index1], tested_labels[index2]);
            if (model_output == tested_output) {
                passed_tests++;
            } else if (debug) {
                std::cerr << model_output << " " << tested_output << "\n";
...
void correctness_tester::run_tests() {
    ...
    for (int test_case = 0; test_case < RANDOM_NUMBER_OF_TEST_CASES;
         test_case++) {
        ...
        if (100 * (test_case + 1) / RANDOM_NUMBER_OF_TEST_CASES >
            percent_done) {
    ...
    for (int test_case = 0; test_case < RANDOM_NUMBER_OF_TEST_CASES;
         test_case++) {
        std::pair<int, int> naive_random_test =
            generate_random_operations(new naive(), new parsings(1998, false),
                                       RANDOM_NUMBER_OF_OPERATIONS);
        total_passed += naive_random_test.first;
        total_tests += naive_random_test.second;
        if (naive_random_test.first == 0) {
